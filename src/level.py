...
from game import Piece, PieceState, execute_move, Victory, Defeat, MoveDirection, GameState, AiLevel


def game_loop(gamestate: GameState | int, screen: Surface, size) -> int:
    number_moves: int = 0
    font = pygame.font.Font("../src/assets/fonts/PressStart2P-Regular.ttf", 20)
    while True:
        if isinstance(gamestate, GameState):
            game_display(gamestate, screen, size)

        moves_text = font.render(
            f"Moves: {number_moves}", True, (255, 255, 255))
        ...
        if gamestate == 0:
            return number_moves
        elif isinstance(gamestate, GameState) and Victory(gamestate):
            display_endgame(screen, 0)
            time.sleep(3)
            return number_moves
        elif isinstance(gamestate, GameState) and Defeat(gamestate):
...
def game_display(gamestate: GameState, screen: Surface, size: tuple[int, int]) -> None:
    ...
    player_blockh1 = pygame.image.load(
        "../src/assets/images/player_blockh1.png")

    player_blockh2 = pygame.image.load(
        "../src/assets/images/player_blockh2.png"
    )

    player_blockv2 = pygame.image.load(
        "../src/assets/images/player_blockv2.png"
    )

    # Vertical display of cube
    player_blockv1 = pygame.image.load(
        "../src/assets/images/player_blockv1.png")
...
def game_move(gamestate: GameState | int) -> GameState | int:
    if (isinstance(gamestate, int)):
        print("game_move received an int as gamestate")
        sys.exit(1)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        gamestate = execute_move(
                            gamestate, MoveDirection.LEFT, False)[0]
                        done = True
                    elif event.key == pygame.K_RIGHT:
                        gamestate = execute_move(
                            gamestate, MoveDirection.RIGHT, False)[0]
                        done = True
                    elif event.key == pygame.K_UP:
                        gamestate = execute_move(
                            gamestate, MoveDirection.UP, False)[0]
                        done = True
                    elif event.key == pygame.K_DOWN:
                        gamestate = execute_move(
                            gamestate, MoveDirection.DOWN, False)[0]
                        done = True
    return gamestate


def display_endgame(screen, w_or_l: int) -> None:
    # print(w_or_l)
    if w_or_l == 0:
...
def start_game(filepath: str, isAi: bool, mode: AiLevel = AiLevel.BFS):
    ...
    firstPortal: tuple[int, int] | None = None
    secondPortal: tuple[int, int] | None = None
    ...
    if (firstPortal is None and (secondPortal is not None)) or \
        (secondPortal is None and (firstPortal is not None)):
    ...
    if isAi:
        initialGameState = GameState(
            board, piece, portalMap, isAi=True, aiLevel=mode)
